...
struct LazyList<T> {
    ...
    func elememt(at index: Int) throws -> T {
        guard useCache else {
    ...
    guard let elememt = try access(index) else {
...
extension LazyList: Sequence {
    enum Error: LocalizedError {
        ...
        var localizedDescriptio: String {
            
            switch self {
            case let .elementISNil(index):
                return "Element  ad index \(index) is nil"
    ...
    struct Iterator: IteratorProtocol {
        ...
        mutating func next() -> Element? {
            ...
            do {
                ...
                return try list.elememt(at: index)
            }
            catch _ {
